GUIDE D'INSTALLATION ET DE CONFIGURATION DE LA PLATEFORME DE DOCUMENTATION (DSPACE 9.x)
Projet : Centre de Documentation
Version DSpace : 9.x (Backend)
Serveur Cible : Ubuntu/Debian 
...
Connectez-vous en tant que votre utilisateur système (ex: user).
...
CREATE USER dspace WITH PASSWORD 'DSpace2024!Secure';
...
Entrez le mot de passe : DSpace2024!Secure
...
Votre backend est maintenant accessible à l'adresse : http://nomde.domaine.cm:8080/server
...
Accédez à l'interface DSpace en tant qu'administrateur (via nomde.domaine.cm:4000 ou l'URL de test).
...
Ouvrez votre navigateur et allez sur l'interface d'exploration de l'API (HAL Browser) : URL : http://nomde.domaine.cm:8080/server Vous devriez voir une interface technique avec des liens.
Test des Entités : Cherchez le lien api/config/entitytypes et cliquez dessus (ou allez directement sur http://nomde.domaine.cm:8080/server/api/core/entitytypes). Ce que vous devez voir : Dans la liste JSON qui s'affiche, vous devez trouver vos entités personnalisées : ProjetAAR, Institution, Auteur. Si vous ne les voyez pas, c'est que le fichier item-submission.xml n'a pas été pris en compte.
Test des Métadonnées : Allez sur http://nomde.domaine.cm:8080/server/api/core/metadatafields/search/byFieldName?schema=cdnss&element=projet&qualifier=status 
...
1.	L'URL de l'API : http://nomde.domaine.cm:8080/server
